...
Our results show that \todo{results }
...
\begin{itemize}
    ...
    \item As shown in \cite{chen_nuqclq_2021}, in most iterations there are many possible swaps with the same gain, but not all of them lead to the same local optima. To decide between swaps with equivalent gain the authors propose to use a more fine-grained secondary scoring function that serves as a tiebreaker. 
\end{itemize}
...
Therefore, we investigate both the application of centrality-based and learning-based feature initialization methods. 
...
In addition to the node embeddings and the context embedding, the current $d_S$-values can also be used as decoder inputs. Furthermore, as the decoder evaluates each vertex independently, it might also be relevant to add information about the distribution of the $d_S$ values, e.g. mean and standard deviation. 
...
Moreover, all models are trained with a look-ahead depth $d=1$, as we want to find out which node features are suited to imitate such a look-ahead search before trying higher and thus more computationally expensive values for look-ahead depth. 
...
The results obtained on 20 random graphs and on benchmark instance brock\_200\_2, which has a similar size and density, are shown in Figure~\ref{fig:V200-node-features}.  
%and while the differences seem marginal, we notice slightly higher mean and more stable results for D+N2V, E1, and N2V+S2V. 

To investigate the performance of the models on high-density graphs, we conduct a second experiment with graphs of fixed size $|V|=450$ and a density $\mathcal{D} \sim \mathcal{0.82, 0.825}$, which corresponds to the size and densities of the benchmark instances frb30-15-$i, 1\leq i \leq 5$. 
...
\subsection{Robustness towards variance in inputs}
