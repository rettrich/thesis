...
\usepackage[acronym,toc,symbols]{glossaries} % Enables the generation of glossaries and lists of acronyms. This package has to be included last.
...
\newglossaryentry{gamma}{%
  ...
  description={Input parameter $\gamma$ for the Maximum Quasi-Clique Problem. },
  type=symbols
}

\newglossaryentry{r}{%
  name={\ensuremath{r}},
  description={Root node in the beam search based lower bound heuristic.},
  type=symbols
}

\newglossaryentry{beam}{%
  name={\ensuremath{\mathcal{B}}},
  description={Set of nodes representing the beam in the beam search based lower bound heuristic.},
  type=symbols
}

\newglossaryentry{blocked}{%
  name={\ensuremath{B}},
  description={Set of nodes that are blocked by the short-term memory mechanism during local search.},
  type=symbols
}

\newglossaryentry{c}{%
  name={\ensuremath{c}},
  description={Parameter for construction heuristic that controls the balance between exploration and GRASP-like construction steps, $c \in [0,1]$.},
  type=symbols
}

\newglossaryentry{tau}{%
  name={\ensuremath{\tau}},
  description={User-defined timelimit for the execution of LSBM.},
  type=symbols
}

\newglossaryentry{eta}{%
  name={\ensuremath{\eta}},
  description={The execution of the local search procedure is restarted after $\eta$ swaps without improvement.},
  type=symbols
}

\newglossaryentry{xi}{%
  name={\ensuremath{\xi}},
  description={The execution of LSBM is stopped after $\xi$ restarts without finding a feasible solution.},
  type=symbols
}

\newglossaryentry{a}{%
  name={\ensuremath{a}},
  description={Aspiration value during the local search procedure.},
  type=symbols
}

\newglossaryentry{z}{
    name={\ensuremath{z}},
    description={The number of iterations of the training algorithm LSBM-T.},
    type=symbols
}

\newglossaryentry{beta}{
    name={\ensuremath{\beta}},
    description={The beam width of the beam search based lower bound heuristic.},
    type=symbols
}

\newglossaryentry{epsilon}{
    name={\ensuremath{\varepsilon}},
    description={The maximum number of successor node of a node in the search tree during the execution of the beam search based lower bound heuristic.},
    type=symbols
}

\newglossaryentry{kprime}{
    name={\ensuremath{k^{\prime}}},
    description={The size of the restricted candidate sets $X,Y$ during local search when using a GNN-based scoring function.},
    type=symbols
}

\newglossaryentry{kstar}{
    name={\ensuremath{k^*}},
    description={The size of the restricted candidate sets $X,Y$ during computation of the target values using the look-ahead search.},
    type=symbols
}

\newglossaryentry{rho}{
    name={\ensuremath{rho}},
    description={The maximum capacity of the replay buffer used in LSBM-T.},
    type=symbols
}

\newglossaryentry{le}{
    name={\ensuremath{l_e}},
    description={The number of layers used in the encoder.},
    type=symbols
}

\newglossaryentry{ld}{
    name={\ensuremath{l_d}},
    description={The number of layers used in the decoder.},
    type=symbols
}

\newglossaryentry{dimeconv}{
    name={\ensuremath{\mathit{dim}_{e}^{\mathit{conv}}}},
    description={The dimension of the convolutional layers of the encoder.},
    type=symbols
}

\newglossaryentry{dimeff}{
    name={\ensuremath{\mathit{dim}_{e}^{\mathit{FF}}}},
    description={The dimension of the feed-forward layers of the encoder.},
    type=symbols
}

\newglossaryentry{dimd}{
    name={\ensuremath{\mathit{dim}_{d}}},
    description={The dimension of the feed-forward layers of the decoder.},
    type=symbols
}

\newglossaryentry{d}{
    name={\ensuremath{d}},
    description={The depth of the look-ahead search.},
    type=symbols
}
...
\setdate{05}{12}{2022} % Set date with 3 arguments: {day}{month}{year}.
...
\begin{abstract}
This thesis investigates the utilization of Graph Neural Networks (GNNs) in a local search-based metaheuristic for edge-based relaxations of the Maximum Clique Problem (MCP), namely, the Maximum Quasi-Clique Problem (MQCP) and the Maximum Defective Clique Problem (MDCP). 
The MCP is the problem of finding fully connected subsets of vertices of maximum size in a graph. It is a well-studied, fundamental NP-hard combinatorial optimization problem and has numerous applications in, e.g., bioinformatics and social network analysis. 
Some real-world applications however require relaxations of the clique model. Thus, researchers have come up with the MQCP and MDCP as a theoretical model for such applications. 

Due to the computational complexity of the considered problems, real-world instances are often infeasible to solve in practice using exact methods. 
Therefore, we develop a heuristic solution approach that is centered around local search, like most state-of-the-art algorithms for the MQCP and MDCP. 
We study and review the relevant literature and build upon the leading (meta)heuristic algorithms for the MQCP. 

The first main contribution of this thesis is a local search-based metaheuristic named LSBM that can utilize a GNN as a scoring function that is used to restrict the neighborhoods during the local search procedure to the most promising vertices. 
Secondly, we propose a training algorithm LSBM-T that can be used to generate training data that can be used to train the GNN to predict, which vertices in a neighborhood will most likely lead to an improved solution, by imitating a more expensive exhaustive search of a possibly larger neighborhood. 

We evaluate our methodology by conducting several computational experiments regarding the parameters of LSBM and LSBM-T. We investigate different feature initialization methods, robustness towards variance of input data, and how well our method generalizes to larger, unseen instances. 
Our results show that 

\todo{paragraph about results}
...
% When reviewing the results reported by these authors, we notice that -- due to the exploitation of the heredity of the problem -- exact algorithms for the MDCP can solve much larger graphs in practice in an exact manner. 
...
\begin{algorithm}
    \DontPrintSemicolon
    \KwIn{Graph $G$, $n$-element array $Freq$, Target size $k$, GRASP parameter $\alpha$, Exploration parameter $b$}
...
\begin{algorithm}
    ...
    \For{$z$ iterations}{
        ...
        $\mathcal{S} \gets $ reconstruct $n_s$ randomly sampled candidate solutions from $H$ \; \label{alg:training-sampling}
        \For{$S \in \mathcal{S}$}{
...
The computational experiments are focused on evaluating results obtained on graphs of high density, i.e., the number of neighbors of each vertex is in $O(n)$. 
These instances seem especially challenging, as exact algorithms are infeasible to use in practice on high-density graphs. We consider both random graphs and selected benchmark instances within the scope of our evaluation. Moreover, we primarily focus on results for MQCP instances, as there is a lack of heuristic solution approaches for the MDCP to compare the performance of our approach. 
...
\begin{table}
    ...
    \begin{tabular}{ll}
        ...
        $b$ & Parameter $b$ for construction heuristic \\
...
The benchmark instances used in this Section are shown in Table~\ref{tab:benchmark-instances-for-experiments} for reference.
...
We evaluate the effect on runtime and solution quality by comparing the performance of the values $k^\prime \in \{10,20,30\}$. 
...
To reduce this overhead, we propose to only apply the decoder if the last vertex swap did not yield a solution with an increased objective value. 
Figure~%\ref{fig:sparse-evaluation} 
shows the results of this approach. \todo{show results}

% \subsection{Short-Term Memory Type}
% We evaluate and compare two different short-term memory types within LSBM that are used in state-of-the-art heuristic algorithms for the MQCP: Tabu Lists (\cite{djeddi_extension_2019}, \cite{zhou_opposition-based_2020}) and Configuration Checking (\cite{chen_nuqclq_2021}). 
% The tabu list is defined by a single parameter, called tabu tenure, which indicates the number of iterations a vertex is blocked after a swap. The authors of \cite{zhou_opposition-based_2020} recommend a setting of 

\section{Results on Benchmark Instances}\label{sec:benchmark-results}
For the final experiments on the benchmark instances, we choose one centrality-based node feature (E1), one learning-based node feature (N2V+S2V), and one node feature that combines both approaches (E1+S2V), as these features have shown to be promising during our evaluation. 
The remaining parameter settings during training are exactly as described in Section \ref{sec:lsbm-t}. 
We train ten randomly initialized GNNs on representative instances for each evaluated benchmark instance or instance group. Training instances are generated with the same size and density as the corresponding instances or instance groups. After the training is completed, we adopt the GNN that yields the best results on 20 unseen uniform random graphs of similar size and density as those seen during training. 
...
\begin{sidewaystable}
    ...
    \begin{tabular}{llrrrr | rrr rrr rrr r}
        \toprule
        \multicolumn{6}{c}{ } & \multicolumn{3}{c}{LSBM (E1)} & \multicolumn{3}{c}{LSBM (N2V+S2V)} & \multicolumn{3}{c}{LSBM (E1+S2V)} &  \\ \cmidrule{7-9} \cmidrule{10-12} \cmidrule{13-15}
        Benchmark & Instance & $|V|$ & $|E|$ & $\mathrm{dens}(G)$ & $\gamma$ & Best & Avg & T(s) & Best & Avg & T(s) & Best & Avg & T(s) & Best LB\\ \hline
        DIMACS & brock200\_1 & $200$ & $14834$ & $0.745$ & 0.800 & 114 & 114.0 & 49.46 & 114 & 114.0 & 41.21 & 114 & 114.0 & 42.42 & 114\\
        DIMACS & brock200\_2 & $200$ & $9876$ & $0.496$ & 0.800 & 24 & 24.0 & 57.46 & 24 & 24.0 & 52.99 & 24 & 24.0 & 55.82 & 24\\
        DIMACS & brock200\_3 & $200$ & $12048$ & $0.605$ & 0.800 & 41 & 41.0 & 26.14 & 41 & 41.0 & 51.50 & 41 & 41.0 & 32.39\\
        % DIMACS & brock400\_1 & $400$ & $59723$ & $0.748$\\
        % DIMACS & brock400\_2 & $400$ & $59786$ & $0.749$\\
        % DIMACS & brock400\_3 & $400$ & $59681$ & $0.748$\\
        DIMACS & C125 & $125$ & $6963$ & $0.898$ & 0.999 & 34 & 34.0 & 7.308 & 34 & 34.0 & 7.32 & 34 & 34.0 & 7.05 & 34\\
        DIMACS & C250 & $250$ & $27984$ & $0.899$ & 0.999 & 44 & 44.0 & 24.28 & 44 & 44.0 & 32.26 & 44.0 & 43.4 & 24.43 & 44\\
        % DIMACS & C500 & $500$ & $112332$ & $0.900$
        % DIMACS & DSJC500 & $500$ & $62624$ & $0.502$\\
        ...
        DIMACS & MANN\_a9 & $45$ & $918$ & $0.927$ & 0.999 & 16 & 16.0 & 5.73 & 16 & 16.0 & 5.82  & 16 & 16.0 & 5.83 & 16 \\
        % BHOSLIB & frb30-15-1 & $450$ & $83198$ & $0.824$\\
        % BHOSLIB &frb30-15-2 & $450$ & $83151$ & $0.823$\\
        % BHOSLIB &frb30-15-4 & $450$ & $83194$ & $0.823$\\
        % BHOSLIB &frb30-15-5 & $450$ & $83231$ & $0.824$\\
...
In the context of this algorithm, the GNN is used to restrict the neighborhood of a candidate solution to only the most promising vertices in and outside of the current candidate solution. 
...
Finally, we evaluate selected benchmark instances and compare our approach to the state-of-the-art. The results show that 
in terms of solution quality, the performance of the models trained using LSBM-T gets close to the leading methods but is not yet competitive. 
Moreover, in terms of runtime, our method is outperformed by the leading methods, as the computational overhead introduced by the GNN is simply too large. 
...
During our evaluation, we notice there is potential for the improvement of generalization to larger instances. In this context, future work shall evaluate curriculum learning strategies as evaluated in \cite{Lisicki2020}, which might lead to better generalization. 
