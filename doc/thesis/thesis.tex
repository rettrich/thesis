...
The authors conclude that centrality-based approaches are generally not as suitable as learning-based approaches. 
Especially DeepWalk seems to be a promising feature initialization method for our purpose, as the authors of \cite{Duong2019} obtain the best results using this method. Furthermore, DeepWalk's generalization Node2Vec \cite{GroverL16} should be of interest as well.  

\subsection{Encoder}
In general, any graph convolutional network model can be used as an encoder. We refer to Residual Gated Graph Convolutional Networks (\cite{Bresson2017}), and GATv2 networks (\cite{Velickovic2018}, \cite{Brody2021}), which we will use in our experiments. 
...
\subsection{Decoder}
The decoder is a regression model that maps the inputs obtained from the node embeddings and the context embedding to scalar values representing scores for the vertices in $V$. We suggest using a simple Multi Layer Perceptron with a sigmoid activation function on the final layer, mapping the results of the previous layer to scalar values between zero and one. 
