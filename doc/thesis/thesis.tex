...
In Figures %\ref{fig:bs-heuristics-random}
we show the obtained solution qualities and runtimes of the three heuristics on randomly generated instances of different sizes and densities. We generated 10 MQCP graph instances for each combination of $|V| \in \{500, 1000\}, dens(G) \in \{0.75, 0.9\}$ with $\gamma=0.95$. 
As to be expected, \emph{Greedy Completion} outperforms the other heuristics in terms of solution quality, but due to its time complexity, it should only be used when smaller solutions are to be expected, as the runtime of the heuristic is dependent on the solution length. 
...
We can also see that the difference in runtime between \emph{Feasible Neighbors} and \emph{Objective Value} is marginal, while the former produces better results in all tested hyperparameter settings. 
...
\begin{figure}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth]{graphics/lbh-500-075-size.pdf}
        \caption{Solution sizes}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[width=\textwidth]{graphics/lbh-500-075-runtime.pdf}
        \caption{Runtime}
    \end{subfigure}
