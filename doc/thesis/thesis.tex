...
\fxnote{RE: Learning-based features: DeepWalk und node2vec sind nicht geeignet für across-network transfer learning tasks - Experimente mit diesen Initialisierungsmethoden waren daher erfolglos. DeepGL könnte vielversprechend sein, aber ich habe keine Implementierung gefunden - da ist es die Frage ob es den Aufwand wert ist?}
...
\fxnote{RE: Vielleicht noch eine lineare Transformation (Dense layer), vor layer 1 einfügen?}
...
\fxnote{RE: in \cite{Kool2019} wird im Decoder auch ein Attention layer verwendet, vermutlich gibt es hier noch bessere Varianten}
