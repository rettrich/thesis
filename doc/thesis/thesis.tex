...
In order to make the training process more stable, introduce a user-defined parameter $\mathit{warmup_it} \in \mathbb{N}$ to the look-ahead search. This parameter dictates the number of iterations that the scoring function $d_S$ is used to restrict the neighborhood during the look-ahead search instead of the scoring function $\mathit{gnn}_S$, as the latter is initially untrained and thus unlikely to restrict the neighborhood to the most promising vertices. After the first $\mathit{warmup_it}$ iterations of the LSBM-T are completed, the look-ahead search then uses the scoring function $\mathit{gnn}_S$ to restrict the neighborhood. 
