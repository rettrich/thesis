...
Resets the short term memory and initializes it with a graph. 
Memory about blocked vertices is reset. Should be called at the start of each local search 
procedure

- `short_term_memory`: Short term memory to be reset

"""
reset!(stm::ShortTermMemory, graph::SimpleGraph) = 
...
"""
get_blocked(stm::ShortTermMemory)::Set{Int} = 
...
- `graph`: Current graph that is being operated on, has to be set manually using `reset!`
...
mutable struct TabuList <: ShortTermMemory 
    ...
    function TabuList(tabu_tenure::Int = 10)
        new(nothing, tabu_tenure, [], 0)
    end
end

function reset!(stm::TabuList, graph::SimpleGraph)
    stm.graph = graph
    stm.tabu_list = fill(0, nv(graph))
    stm.iteration = 0
    nothing
end

function get_blocked(stm::TabuList)::Set{Int}
    Set(filter(v -> stm.tabu_list[v] > stm.iteration , eachindex(stm.tabu_list)))
end
...
mutable struct ConfigurationChecking <: ShortTermMemory 
    ...
    function ConfigurationChecking(ub_threshold::Int = 7)
        new(nothing, ub_threshold, [], [])
    end
end

function reset!(stm::ConfigurationChecking, graph::SimpleGraph)
    stm.graph = graph
    stm.threshold = fill(1, nv(graph))
    stm.conf_change = fill(1, nv(graph))
    nothing
end
