...
parse_settings!([settings_cfg, thesis.NodeRepresentationLearning.settings_cfg],
                ...
                [
                    ...
                    # "--stm=cc",
                    "--timelimit=300.0",
                    "--scoring_function=/mnt/c/Users/Rupert/Desktop/Thesis/thesis/logs/MQCP-GATv2Conv-thesis.GNNs.Dense-128-64-64-64-32-32-feature_set=Node2Vec_2_4-Struct2Vec-V=125.0-1.0-dens=0.895-0.905-2022-12-04T01-23-11jl_wjULI7OhYR.bson",
                    "--graph=inst/DIMACS/C125.9.clq",
...
function run_mdcp(scoring_function=nothing)
    γ = settings[:gamma]
    ...
    guidance_func = MDCP_FeasibleNeighborsHeuristic(false)
    lower_bound_heuristic = BeamSearch_LowerBoundHeuristic(guidance_func; β=5, γ, expansion_limit=10, is_mdcp=true)
    ...
    feasibility_checker = MDCP_FeasibilityChecker(Int(γ))
    
    solution_extender = MDCP_GreedySolutionExtender(Int(γ))
    ...
    local_search = LocalSearchBasedMH(
            ...
            sparse_evaluation=settings[:sparse_evaluation], score_based_sampling, is_mdcp=true
            )
